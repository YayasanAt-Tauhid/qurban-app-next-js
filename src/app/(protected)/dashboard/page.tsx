        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <div>
            ...
            <h3 className="text-xl font-bold">
              10 Dzulhijjah 1447H / 6 Juni 2026
            </h3>
          </div>
          <div className="flex gap-3">
            {[
              ...
              <div
                key={item.label}
                className="text-center bg-primary-foreground/10 rounded-lg px-3 py-2 min-w-[60px]"
              >
  ...
  const { data: hewanStats, isLoading: loadingHewan } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("hewan_qurban")
        .select("id, jenis_hewan, status")
        .eq("tahun", 1447);
      if (error) throw error;
      const rows = (data ?? []) as any[];
      const total = rows.length;
      const sapi = rows.filter((h) => h.jenis_hewan === "sapi").length;
      const kambing = rows.filter((h) => h.jenis_hewan === "kambing").length;
    ...
    queryFn: async () => {
      ...
      return hewanData.map((h) => {
        ...
        BAGIAN_LIST.forEach((b) => {
          counts[b] = requestData.filter(
            (r) => r.hewan_id === h.id && r.bagian === b
          ).length;
        });
        return { ...h, counts };
      });
    ...
    queryFn: async () => {
      ...
      return Object.entries(weeks)
        ...
        .map(([week, count]) => {
          cumulative += count;
          return {
            week: new Date(week).toLocaleDateString("id-ID", {
              day: "2-digit",
              month: "short",
            }),
            total: cumulative,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          title="Total Hewan"
          value={hewanStats?.total ?? 0}
          icon={Beef}
          loading={loadingHewan}
          subtitle={
            hewanStats
              ? `${hewanStats.sapi} sapi, ${hewanStats.kambing} kambing`
              : undefined
          }
        />
        <StatCard
          title="Shohibul Qurban"
          value={shohibulCount ?? 0}
          icon={Users}
          loading={loadingShohibul}
        />
        <StatCard
          title="Mustahiq"
          value={mustahiqCount ?? 0}
          icon={Ticket}
          loading={loadingMustahiq}
        />
        <StatCard
          title="Iuran Terkumpul"
          value={formatRupiah(kasStats?.masuk ?? 0)}
          icon={TrendingUp}
          loading={loadingKas}
        />
        <StatCard
          title="Saldo Kas"
          value={formatRupiah(kasStats?.saldo ?? 0)}
          icon={Wallet}
          loading={loadingKas}
        />
      </div>
      ...
      {rekapData && rekapData.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Rekap Request Bagian</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    {BAGIAN_LIST.map((b) => (
                      <TableHead key={b} className="text-center capitalize">
                        {b}
                      </TableHead>
                    ))}
                  </TableRow>
