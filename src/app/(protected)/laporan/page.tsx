...
const exportRequestBagian = async () => {
  ...
  (hewan as any[]).forEach((h) => {
    ...
    bagianList.forEach((b) => {
      const reqs = (requests as any[]).filter((r) => r.hewan_id === h.id && r.bagian === b);
...
const exportKeuangan = async () => {
  ...
  const masuk = (data as any[]).filter((k) => k.jenis === "masuk").reduce((a, k) => a + Number(k.jumlah), 0);
  const keluar = (data as any[]).filter((k) => k.jenis === "keluar").reduce((a, k) => a + Number(k.jumlah), 0);
